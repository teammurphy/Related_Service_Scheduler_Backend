...
def create_mandate(mandate: mandate_schema.MandateCreate, db: Session = Depends(get_db)):
    return crud.mandate.create_mandate(db=db, mandate=mandate)


@router.get("/mandate/{mandate_id}", response_model=mandate_schema.Mandate, tags=["mandate"])
...
def update_mandate(updated_mandate: mandate_schema.MandateCreate, mandate_id: int, db: Session = Depends(get_db)):
